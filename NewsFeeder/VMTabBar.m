...
-(void)iconTabBarWithNumber:(NSInteger)num
{
    ...
    for (NSInteger i = 0; i < num; i++)
    {
        ...
        UIImageView *circle = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 30, 30)];
        
        CGPoint ct = CGPointMake(((i % num + 0.65) * (self.bounds.size.width / num - 5)),(self.bounds.size.height) - 35);
        circle.center = ct;
...
-(void)selectTabBarValueWithTagForImage:(NSInteger)tag
{
    for (NSInteger i = 0; i<self.listOfItemTabbar.count; i++) {
        UIImageView *imgTab = (UIImageView*)[self.listOfItemTabbar objectAtIndex:i];
