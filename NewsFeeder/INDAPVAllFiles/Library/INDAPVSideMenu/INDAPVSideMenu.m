...
- (id)initWithItems:(NSArray *)items {
    ...
    if (self) {
        ...
        _menuPosition = INDAPVSideMenuPositionRight;
    }
...
- (void)showItem:(UIView *)item {
   [NSObject cancelPreviousPerformRequestsWithTarget:item.layer];
    ...
    if (self.menuIsVertical) {
        position.x += self.menuPosition == INDAPVSideMenuPositionRight ? - self.menuWidth : self.menuWidth;
        
        [self animateLayer:item.layer
               withKeyPath:@"position.x"
                        to:position.x];
    } else {
        position.y += self.menuPosition == INDAPVSideMenuPositionTop ? self.menuHeight : - self.menuHeight;
...
- (BOOL)menuIsVertical {
    if (self.menuPosition == INDAPVSideMenuPositionLeft || self.menuPosition == INDAPVSideMenuPositionRight)
        return YES;
...
- (void)layoutSubviews {
   ...
   // Calculate the menu size
    ...
    { //if (self.position == HMSideMenuPositionTop || self.position == HMSideMenuPositionBottom) {
        [self.items enumerateObjectsUsingBlock:^(UIView *item, NSUInteger idx, BOOL *stop)
         {
    ...
    if (self.menuIsVertical) {
        x = self.menuPosition == INDAPVSideMenuPositionRight ? self.superview.frame.size.width : 0 - self.menuWidth;
        ...
        y = self.menuPosition == INDAPVSideMenuPositionTop ? 0 - self.menuHeight : self.superview.frame.size.height;
    }
