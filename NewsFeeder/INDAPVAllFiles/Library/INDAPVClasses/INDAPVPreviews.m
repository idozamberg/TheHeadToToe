...
@implementation INDAPVPreviews

#pragma mark Properties

@synthesize delegate;

#pragma mark INDAPVPreviews instance methods
...
- (void)requeueThumbCell:(INDAPVPreview *)tvCell
{
...
- (void)requeueAllThumbCells
{
	if (thumbCellsVisible.count > 0)
	{
		...
		for (INDAPVPreview *tvCell in visible)
		{
...
- (INDAPVPreview *)dequeueThumbCellWithFrame:(CGRect)frame
{

	INDAPVPreview *theCell = nil;
	...
	else // Allocate a brand new thumb cell subclass for our use
	{
		...
		assert([theCell isKindOfClass:[INDAPVPreview class]]); // Check
...
- (INDAPVPreview *)thumbCellContainingPoint:(CGPoint)point
{

	INDAPVPreview *theCell = nil;

	for (INDAPVPreview *tvCell in thumbCellsVisible)
	{
...
- (void)layoutSubviews
{
	...
	if (CGSizeEqualToSize(_lastViewSize, self.bounds.size) == false)
	{
		...
		for (INDAPVPreview *tvCell in thumbCellsVisible) // Enumerate visible cells
		{
		...
		for (INDAPVPreview *tvCell in requeueCells) // Enumerate requeue cells
		{
		...
		[visibleIndexSet enumerateIndexesUsingBlock: // Enumerate visible indexes
			^(NSUInteger index, BOOL *stop)
			{
				...
				INDAPVPreview *tvCell = [self dequeueThumbCellWithFrame:thumbRect];
...
- (void)handleTapGesture:(UITapGestureRecognizer *)recognizer
{
	...
	if (recognizer.state == UIGestureRecognizerStateRecognized) // Handle the tap
	{
		...
		INDAPVPreview *tvCell = [self thumbCellContainingPoint:point]; // Look for cell
